import { X,MapPin,Camera,Bus,Check,RotateCcw,ExternalLink,FileText } from 'lucide-react';
import type { UrbanEvent,Status } from '../types';
const label=(s:string)=>s.replace(/\b\w/g,c=>c.toUpperCase());

export function EventPanel({event,onClose,onAction}:{event:UrbanEvent;onClose:()=>void;onAction:(id:string,s:Status)=>void}){
  return <section className="event-panel">
    ...
    <div className="panel-actions">
      <button className="button primary" onClick={()=>onAction(event.id,'verified')}><Check size={15}/> Verify</button>
      <button className="button" onClick={()=>onAction(event.id,'rejected')}><X size={15}/> Reject</button>
      <button className="button" onClick={()=>onAction(event.id,'resolved')}><RotateCcw size={15}/> Resolve</button>
    </div>
